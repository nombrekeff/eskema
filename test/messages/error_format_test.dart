...
void main() {
  t.group('error_format', () {
    ...
    t.test('buildValidationMessage invalid single', () {
      final r = Result.invalid('x',
          expectation:
              const Expectation(message: 'must be number', value: 'x'));
      final msg = buildValidationMessage(r);
    ...
    t.test('extension detailed()', () {
      final r = Result.invalid(5,
          expectation: const Expectation(message: 'boom', value: 5));
      t.expect(r.detailed(),
          'Validation failed (errors: 1) for value (int): 5\n  1) boom');
    });
  });
}
