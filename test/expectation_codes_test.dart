...
void main() {
  group('Expectation code & data', () {
    ...
    test('structure.unknown_key code from eskemaStrict', () {
      final r = eskemaStrict({'name': isString()})
          .validate({'name': 'a', 'extra': 1});
      expect(r.isValid, false);
    ...
    test('structure.map_field_failed default code when child has none', () {
      final custom =
          validator((_) => false, (v) => Expectation(message: 'bad', value: v));
      ...
      expect(
          r.firstExpectation.code, anyOf('structure.map_field_failed', isNull));
    });
    ...
    test('structure.list_item_failed default code when child has none', () {
      ...
      expect(r.isValid, false);
      expect(
          r.firstExpectation.code, anyOf('structure.list_item_failed', isNull));
    });

    test('copyWith preserves and overrides code/data', () {
      final e =
          const Expectation(message: 'm', value: 1, code: 'c1', data: {'a': 1});
    ...
    test('toJson includes code, path, data', () {
      final e = const Expectation(
          message: 'm', value: 5, code: 'x', data: {'k': 'v'}, path: 'p');
      final j = e.toJson();
      expect(j['code'], 'x');
      expect(j['path'], 'p');
