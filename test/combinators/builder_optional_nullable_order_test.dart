  ...
  group('Builder optional/nullable position invariance', () {
    test('optional applied mid-chain still skips missing key', () {
      ...
      final fieldValidator =
          eskema.builder().string().optional().lengthMin(2).build();
      final sch = eskema.eskema({'name': fieldValidator});
      // Missing key should be valid (skip) once behaviour is fixed
      expect(sch.validate({}).isValid, isTrue,
          reason: 'optional() mid-chain should mark full chain optional');
    });

    test('nullable applied mid-chain still accepts null', () {
      final v = eskema.builder().string().nullable().lengthMin(2).build();
      final rNull = v.validate(null);
      expect(rNull.isValid, isTrue,
          reason: 'nullable() mid-chain should allow null');
      final rShort = v.validate('a');
      expect(rShort.isValid, isFalse,
          reason: 'subsequent constraints still enforced for non-null');
    ...
    test('optional + nullable mid-chain both honoured', () {
      final v =
          eskema.builder().string().nullable().optional().lengthMin(2).build();
      final sch = eskema.eskema({'nickname': v});
      // Missing key => skip (optional)
      expect(sch.validate({}).isValid, isTrue,
          reason: 'missing key should be skipped');
      // Null value (present) => valid (nullable)
      expect(sch.validate({'nickname': null}).isValid, isTrue,
          reason: 'null present should be valid');
    ...
    test(
        'ordering invariance: optional before vs after later constraints are equivalent',
        () {
      final midChainOptional = eskema
          .builder()
          .string()
          .optional()
          .lengthMin(2)
      ...
      final sch1 = eskema.eskema({'x': midChainOptional});
      final sch2 = eskema.eskema({'x': tailOptional});

      // Both should treat missing key the same
      expect(sch1.validate({}).isValid, isTrue,
          reason: 'mid-chain optional should skip');
      expect(sch2.validate({}).isValid, isTrue,
          reason: 'tail optional should skip');
    ...
    test(
        'ordering invariance: nullable before vs after later constraints are equivalent',
        () {
      final midChainNullable = eskema
          .builder()
          .string()
          .nullable()
          .lengthMin(2)
    ...
    test('idempotency: multiple optional/nullable calls don\'t break semantics',
      ...
      final v = eskema
          ...
          .nullable()
          .build();
      final sch = eskema.eskema({'field': v});
      expect(sch.validate({}).isValid, isTrue, reason: 'still optional');
      expect(sch.validate({'field': null}).isValid, isTrue,
          reason: 'still nullable');
      expect(sch.validate({'field': 'a'}).isValid, isFalse,
          reason: 'constraints still enforced');
