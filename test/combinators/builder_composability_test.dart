...
void main() {
  group('Builder composability as IValidator', () {
    ...
    test(
        'none with builder child collecting valid expectations (builder pass triggers none failure)',
        () {
      final v = none([
      ...
      expect(v.validate('ok').isValid, true,
          reason: 'neither child passes => none succeeds');
      expect(v.validate('block').isValid, false,
          reason: 'first child passes => none fails');
    });
    ...
    test(
        'builder nullable mid-chain still nullable when composed (desired future behaviour)',
        () {
      ...
      expect(rNull.isValid, true,
          reason: 'null should be accepted once mid-chain nullable is fixed');
      expect(esk.validate({'name': 'a'}).isValid, false);
      expect(esk.validate({'name': 'ab'}).isValid, true);
    });

    test('async composition: builder & async validator', () async {
      final asyncFailing = Validator((v) async =>
          Result.invalid(v, expectation: const Expectation(message: 'bad')));
