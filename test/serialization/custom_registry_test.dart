...
void main() {
  ...
  final myPalindromeValidator = Validator((value) {
    if (value is! String) {
      return Result.invalid(
        value,
        expectation: const Expectation(message: 'must be string'),
      );
    }
    ...
    if (value != reversed) {
      return Result.invalid(
        value,
        expectation: const Expectation(message: 'must be a palindrome'),
      );
    }
  ...
  final serialized = const EskemaEncoder(customSymbols: {'isPalindrome': 'palin'})
      .encode(myPalindromeValidator, registry: customRegistry);
  ...
  final deserialized = const EskemaDecoder(customSymbols: {'palin': 'isPalindrome'})
      .decode(serialized, registry: customRegistry);
  test('Custom functionality successfully tested! Registry injected flawlessly.', () {
